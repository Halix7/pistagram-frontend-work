...
const PinDetails = ({ user }) => {
  ...
  const addComment=()=>{
    if(comment){
      ...
      client.patch(pinId).setIfMissing({
        comments:[]
  ...
  return (
    ...
    >
      ...
      <Link
      to={`user-profile/${pinDetail.postedBy?._id}` } className="flex gap-2 mt-5 items-center bg-white"
      >
    ...
    </div>
    {
      pins?.length > 0 ? (
        ...
        <Spinner message="Loading more Assets..."/>
      )
    }
      </>
  );
